...
- The target host and port are injected into `resources-ms` from the `resources-ms-config` ConfigMap (`k8s-helm-chart/templates/1.6-resources-config.yaml`):
  - `SONGS_MS_URL`: `songs-ms` (matches the `Service` name in `k8s-helm-chart/templates/5-song-ms.yaml`)
  - `SONGS_MS_PORT`: `8081`
